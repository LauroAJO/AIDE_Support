...
// compacta (ver mesma função em HubPage.jsx).
...
export default function VagasPhDPage({ refreshToken = 0, highlightShortId = null, onCountChange = () => {} }) {
  ...
  // v2.26.4 — alternância Cards/Lista (Bloco 4B). A Lista é uma tabela
  // compacta com colunas ordenáveis; usa o mesmo `filtered` do modo Cards.
  // v2.26.6 (Bloco 5E) — default trocado de 'cards' para 'list': a Lista é
  // mais compacta e mostra mais itens sem scroll (pedido explícito do spec).
  const [view, setView] = useState('list'); // cards | list
  const [sortKey, setSortKey] = useState('date'); // title | institution | country | relevancia | area | date
  ...
  const load = async () => {
    ...
    try {
      ...
      // v2.26.7 (contador da aba) — dbTotal já É o total real de phd_vagas no
      // banco (via /api/hub/stats), não afetado pelos filtros locais de
      // busca/país/área — é exatamente o número que o spec pediu.
  ...
  const toggleSort = (key) => {
    ...
    setSortKey(key);
    setSortDir(key === 'title' || key === 'institution' || key === 'country' ? 'asc' : 'desc');
  };
  const sortedForList = useMemo(() => {
    const val = (it) => {
      switch (sortKey) {
        ...
        case 'country': return it._country || '';
  ...
  return (
    <div className="mx-auto flex h-full max-w-7xl flex-col gap-3">
      {/* v2.26.6 (Bloco 5B/5C/5G) — título de página, cards de estatísticas
          (Total no banco/Carregados/Novas hoje/Países) removidos por
          completo: o título duplicava a aba já ativa na barra do Hub, e as
          estatísticas ficam resumidas numa única linha compacta abaixo dos
          filtros. Filtros + toggle Cards/Lista agora numa única barra. */}
      ...
      {/* Exibindo X de Y · Z países (substitui os 4 stat cards) */}
      {!loading && !error && total > 0 && (
        <p className="-mt-1 text-xs text-muted">
          Exibindo {items.length} de {total} vagas · {stats.paises} {stats.paises === 1 ? 'país' : 'países'}
        </p>
      )}
...
// v2.26.7 (Change 2b) — variante `compact` usada só no VagaCard (Cards
// view), para não engordar o botão de volta no card já compactado; o modal
// de detalhe continua usando o tamanho padrão (full).
  ...
  if (state === 'done') {
    return (
      <span className={`${base} bg-emerald-100 text-emerald-700`}>
        <CheckCircle2 className={iconClass} /> {compact ? 'Adicionada' : 'Adicionada'}
      </span>
    );
  }
  return (
...
function VagaCard({
  ...
  // v2.26.7 (Change 2b — cards mais compactos): resumo cortado bem mais
  // curto (era 150 chars, multi-linha) e renderizado em 1 linha só
  // (line-clamp-1) — era o maior contribuinte para a altura do card.
...
// v2.26.4 (Bloco 4B) — modo Lista: tabela compacta e ordenável. Colunas do
// spec eram Título/Instituição/País/Relevância/Status/Data/Ações; hub_items
// não tem campo "status" (não são oportunidades no Kanban, só coletas do
// Intelligence Hub) — a coluna equivalente aqui é "Área" (auto-detectada),
// que já existia como badge nos cards. Ações usa os mesmos botões do card:
// Ver, Vincular à Tarefa e Adicionar à Carreira.
// v2.26.7 (Change 1 — linhas compactas): cabeçalho e linhas encolhidos —
// py-2 → py-1, texto em text-[11px], data DD/MM, relevância como número
// simples (sem badge).
// v2.26.7 (fix pós-feedback — faltava o botão de deletar): a Lista tinha
// perdido o ícone de lixeira que o VagaCard sempre teve (onDelete abre o
// mesmo modal de confirmação via setConfirmItem) — reintroduzido aqui como
// o 4º ícone da coluna Ações, mesmo padrão do card.
...
function VagasListTable({ items, sortKey, sortDir, onSort, onOpen, onAdd, added, onLinkTask, onDelete, deletingId }) {
  if (items.length === 0) {
    return (
      <div className="rounded-xl border border-line bg-surface px-4 py-12 text-center text-sm text-muted">
        Nenhuma vaga corresponde aos filtros.
      ...
      <table className="w-full border-collapse text-[11px]">
        <thead className="border-b border-line bg-surface2/60">
          <tr>
            ...
            <SortHeader label="País" sortKey="country" active={sortKey === 'country'} dir={sortDir} onSort={onSort} />
          ...
          {items.map((it, i) => {
            ...
            return (
              ...
              >
                ...
                <td className="px-2.5 py-1"><CountryBadge code={it._country} /></td>
